...
for block in data:
  datum = datetime.strptime(block['Date_measurement'],"%Y-%m-%d" if block['Date_measurement'].startswith("2020") else "%d-%m-%Y");
  if datum > last:
      last = datum
...
for block in data:
  ...
  if pc != '' and datetime.strptime(block['Date_measurement'],"%Y-%m-%d" if block['Date_measurement'].startswith("2020") else "%d-%m-%Y") >= twoweek:
